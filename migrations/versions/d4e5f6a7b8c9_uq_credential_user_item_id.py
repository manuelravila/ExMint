...
def upgrade():
    ...
    # empty string instead of a real Plaid item_id.  Stamp each such row with
    # a unique placeholder (CONCAT('__legacy__', id)) so the UNIQUE constraint
    # on (user_id, item_id) can be applied without deleting rows or orphaning
    # related transactions.  Real Plaid item IDs never start with '__'.
    op.execute(
        "UPDATE credential SET item_id = CONCAT('__legacy__', id) WHERE item_id = ''"
    )
