...
- **Reconnect button persists after successful re-authentication**: after completing Plaid's update-mode flow, a successful `transactions_sync` now clears the `requires_update` flag directly, covering both manual syncs and webhook-triggered syncs.
...
---
>>>>>>> f033604c (feat: bulk category assign + fix reconnect button not clearing after re-auth)
