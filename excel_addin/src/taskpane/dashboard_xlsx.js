...
function syncTransactions() {
    ...
    Excel.run(context => {
        ...
        return workbook.load('worksheets').context.sync()
            ...
            .then(() => {
                if (!transactionsTableExists) {
                    console.log('Updating named ranges and applying formulas');
    ...
    return new Promise((resolve, reject) => {
        ...
        context.sync().then(() => {
            ...
            if (existingSheets.length === 0) {
                console.log('No existing sheets found. Fetching template.xlsx');
                const templateUrl = `${window.appConfig.frontEndUrl}/assets/template.xlsx`;
                fetch(templateUrl)
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(arrayBuffer => {
                        ...
                        console.log('Inserting sheets from template');
                        workbook.insertWorksheetsFromBase64(base64String, options);
                        return context.sync().then(() => {
                            console.log('Sheets inserted successfully');
                            resolve();
                        }).catch(error => {
                            console.error('Error during context.sync after insert:', error);
                            reject(error);
                        });
                    })
                    .catch(error => {
            ...
            console.error('Error during initial context.sync:', error);
            reject(error);
        });
    });
}
